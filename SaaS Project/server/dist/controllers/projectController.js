...
function getStylePresetInstruction(stylePreset) {
    switch (stylePreset) {
        case "ugc":
...
function normalizeAdAngle(value) {
    const normalized = normalizePromptText(value).toLowerCase();
...
function getAdAngleInstruction(adAngle) {
    switch (adAngle) {
        case "premium":
...
function pickVariant(items, seed, offset) {
...
function detectPromptCategory(text) {
    ...
    Object.keys(keywordMap).forEach((category) => {
        const score = keywordMap[category].reduce((total, keyword) => total + (text.includes(keyword) ? 1 : 0), 0);
        if (score > bestScore) {
            bestScore = score;
...
function extractFeatureLine(productDescription) {
...
function buildPromptFromDescription({ productName, productDescription, aspectRatio, variation = 0, stylePreset = "auto", adAngle = "auto", }) {
    ...
    const seed = createSeed(`${combinedText}|${String(aspectRatio || "")}|${resolvedStylePreset}|${resolvedAdAngle}|${variation}`);
    const orientation = String(aspectRatio || "").trim() === "16:9" ? "horizontal" : "vertical";
    const framing = String(aspectRatio || "").trim() === "16:9"
...
async function generatePromptWithGroq({ productName, productDescription, aspectRatio, targetLength, variation = 0, stylePreset = "auto", adAngle = "auto", }) {
    ...
    const response = await axios.post("https://api.groq.com/openai/v1/chat/completions", {
        ...
        messages: [
            {
                role: "system",
                content: "You write high-converting prompts for AI image-to-video ads. Return only one polished prompt as a single paragraph. Do not use bullet points, labels, quotation marks, or extra explanation.",
            },
            {
                role: "user",
                content: [
...
async function uploadImageBufferToCloudinary(buffer) {
...
function getLightXOutputDimensions(aspectRatio) {
...
// =========================================================================
export const createDirectVideo = async (req, res) => {
    let tempProjectId;
    ...
    try {
        ...
        (async () => {
            try {
                ...
                console.log(`[Project ${project.id}] Step 1: Running LightX virtual try-on...`);
                // ==========================================
                ...
                const lightxResponse = await axios.post("https://api.lightxeditor.com/external/api/v2/aivirtualtryon", lightxPayload, {
                    headers: {
                        "Content-Type": "application/json",
                        "x-api-key": lightxApiKey,
                    },
                });
                console.log("LightX Initial Response:", JSON.stringify(lightxResponse.data, null, 2));
                // LightX can return { status: "FAIL", body: null, ... } (e.g. credits exhausted)
                const lightxData = lightxResponse.data;
                if (lightxData?.status &&
                    typeof lightxData.status === "string" &&
                    lightxData.status.toUpperCase() === "FAIL") {
                    const statusCode = lightxData.statusCode != null ? String(lightxData.statusCode) : "";
                    const message = typeof lightxData.message === "string" ? lightxData.message : "";
                    const description = typeof lightxData.description === "string"
                ...
                const directOutputUrl = lightxData?.body?.output ||
                ...
                if (directOutputUrl) {
                    ...
                    while (!isLightXDone) {
                        ...
                        const statusResponse = await axios.post("https://api.lightxeditor.com/external/api/v1/order-status", { orderId: orderId }, {
                            headers: {
                                "Content-Type": "application/json",
                                "x-api-key": lightxApiKey,
                            },
                        ...
                        if (statusData?.status === "active" ||
                            ...
                            throw new Error("LightX image generation failed during processing.");
                        }
                    }
                }
                else {
                    throw new Error("LightX did not return an output URL or orderId. Check LightX response.");
                }
                if (!tryOnImageUrl)
                ...
                const contentType = String(lightxImageResp.headers?.["content-type"] || "").toLowerCase();
                const buffer = Buffer.from(lightxImageResp.data);
                if (lightxImageResp.status < 200 || lightxImageResp.status >= 300) {
                    throw new Error(`LightX output download failed (HTTP ${lightxImageResp.status}).`);
                }
                ...
                if (!contentType.startsWith("image/") && !looksLikeImage(buffer)) {
                    throw new Error(`LightX output is not an image (content-type: ${contentType || "unknown"}).`);
                }
                const uploadedLightXImage = await uploadImageBufferToCloudinary(buffer);
                const hostedTryOnImageUrl = getFixedLightXImageUrl(uploadedLightXImage.publicId, project.aspectRatio || aspectRatio);
                ...
                const shouldSkipMagicHour = String(skipMagicHour ?? process.env.SKIP_MAGIC_HOUR ?? "")
                    .toLowerCase()
                    .trim() === "true";
                if (shouldSkipMagicHour) {
                    ...
                    console.log(`[Project ${project.id}] ⏸️ Skipped MagicHour (LightX image saved).`);
                    return;
                }
                // ==========================================
                ...
                const mhInitialResponse = await axios.post("https://api.magichour.ai/v1/image-to-video", mhPayload, {
                    headers: {
                ...
                while (!isDone) {
                    ...
                    const mhStatusResponse = await axios.get(`https://api.magichour.ai/v1/video-projects/${mhVideoId}`, { headers: { Authorization: `Bearer ${mhApiKey}` } });
                    ...
                    if (status === "complete") {
                        ...
                        status === "canceled") {
                        throw new Error(`Magic Hour generation failed with status: ${status}`);
                    }
                }
                // ==========================================
                ...
                console.error("Background Pipeline Error:", bgError.response?.data || bgError.message);
...
export const generatePromptSuggestion = async (req, res) => {
    try {
        ...
        const { productName, productDescription, aspectRatio, stylePreset, adAngle, targetLength = 5, variation = 0, count = 1, } = req.body;
        ...
        const resolvedStylePreset = normalizeStylePreset(stylePreset);
        const fallbackPrompts = Array.from({ length: promptCount }, (_, index) => buildPromptFromDescription({
            productName,
            productDescription,
            aspectRatio,
        ...
        try {
            const prompts = await Promise.all(Array.from({ length: promptCount }, (_, index) => generatePromptWithGroq({
                productName,
                productDescription,
                aspectRatio,
                stylePreset: resolvedStylePreset,
            ...
            console.error("Groq prompt generation failed, using local fallback:", groqError?.response?.data || groqError?.message || groqError);
            return res.json({
